...
final class CliSnmpTest extends TestCase
{
    public function testWalk() : void
    {
        $output = <<<OUT
.1.3.6.1.2.1.2.2.1.2.1000009 = STRING: "Port-Channel9"
.1.3.6.1.2.1.2.2.1.2.2002002 = STRING: "Vlan2002"
.1.3.6.1.2.1.2.2.1.2.2002019 = STRING: "some
multi-line
string"
.1.3.6.1.2.1.2.2.1.2.2002150 = STRING: "Vlan2150"
OUT;

        $result = $this->getSnmp($output)->walk('.1.3.6.1.2.1.2.2.1.2');
    ...
    {
        $this->expectException(SnmpCliError::class);
        $this->expectExceptionMessage('Invalid SNMP version');

        iterable_to_array((new CliSnmp('', '', 0, 0, 0, 'invalid'))->walk('.1.15'));
    }
    ...
    public function testWalkWithUnknownType() : void
    {
        ...
        iterable_to_array($this->getSnmp('.1.6.6.6.666.0 = OPAQUE: AA BB CC')->walk('.1.6.6.6.666'));
    }

    public function testWalkWithCompleteOids() : void
    {
        $output = file_get_contents(__DIR__ . '/data/public.snmpwalk');
        $output = str_replace('.1.6.6.6.666.0 = OPAQUE: AA BB CC', '', (string) $output);
        $result = $this->getSnmp($output)->walkWithCompleteOids('.1.3.6.1');
    ...
    private function getSnmp(string $result) : CliSnmp
    {
        $snmpBulkWalk = $this->createMock(SnmpBulkWalk::class);

        $snmpBulkWalk->expects(self::once())
            ->method('execute')
            ->willReturn($result);

        assert($snmpBulkWalk instanceof SnmpBulkWalk);

        return new CliSnmp('', '', 0, 0, 0, '2c', $snmpBulkWalk);
    }
}
